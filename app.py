...
if st.session_state['user_id'] is not None:
    ...
    for playlist in playlists:
        ...
        with st.expander(f"🎵 {playlist_name}"):
            ...
            selected_song = st.selectbox(
                ...
                key=f"add_song_{playlist_id}_{playlist_name}"
            )
            
            if st.button("Add to Playlist", key=f"add_btn_{playlist_id}_{playlist_name}"):
                # song_id = available_songs.iloc[available_songs.index[selected_song]]['song_id']
                selected_song_title = selected_song.split(' - ')[0].strip()
                song_id = available_songs.loc[available_songs['title'] == selected_song_title, 'song_id'].values[0]
    ...
    for artist in artists:
        ...
        with col2:
            ...
            if st.button(
                btn_emoji,
                key=f"follow_btn_{artist_id}_{artist_name}",
    ...
    def toggle_like(user_id, song_id, song_title):
        ...
        if current_state:
            ...
            st.session_state[f'heart_{song_id}'] = True  # Set heart to liked (❤️)
    ...
    for index, row in songs_df.iterrows():
        ...
        with col4:
            ...
            if st.button(heart_icon, key=f"heart_btn_{song_id}_{index}", on_click=toggle_like, args=(user_id, song_id, song_title)):
                pass